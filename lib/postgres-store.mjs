...
export async function createPostgresStore({ databaseUrl, schemaPath, defaultState }) {
  ...
  async function patchTicket(id, patch, context = {}) {
    ...
    const ticket = applyTicketPatch(current, patch, context);
    await upsertTicket(pool, ticket);
    return ticket;
  }

  async function mergeTickets(id, input, context = {}) {
    const secondaryTicketIds = Array.isArray(input?.secondaryTicketIds) ? input.secondaryTicketIds.map((ticketId) => String(ticketId)) : [];
    const targetIds = [...new Set([String(id), ...secondaryTicketIds])];
    const client = await pool.connect();
    try {
      await client.query("begin");
      if (targetIds.length) {
        await client.query("select id from tickets where id = any($1) for update", [targetIds]);
      }
      const result = await client.query("select * from tickets order by updated_at desc nulls last, created_at desc nulls last");
      const mergeResult = mergeTicketsInCollection(result.rows.map(ticketFromRow), id, input, context);
      for (const ticket of mergeResult.updatedTickets) {
        await upsertTicket(client, ticket);
      }
      await client.query("commit");
      const { tickets: _tickets, updatedTickets: _updatedTickets, ...response } = mergeResult;
      return response;
    } catch (error) {
      await client.query("rollback");
      throw error;
  ...
  async function appendTicketMessage(id, input, context = {}) {
    ...
    const ticket = {
      ...
      conversation: [...(Array.isArray(current.conversation) ? current.conversation : []), message, activity]
    };
    const client = await pool.connect();
    try {
      await client.query("begin");
      ...
      await client.query(
        `insert into ticket_messages (id, ticket_id, message_type, author, body, created_at, data)
         ...
         on conflict (id) do update set
           ...
           created_at = excluded.created_at,
           data = excluded.data`,
        [activity.id, ticket.id, activity.type, activity.author, activity.body, activity.timestamp, activity]
      );
      await client.query("commit");
    } catch (error) {
      await client.query("rollback");
  ...
  async function getCustomer(id) {
    ...
    const result = await pool.query("select * from customers where id = $1 or lower(email) = $2", [String(id), value]);
  ...
  async function getCustomerByEmail(email) {
    ...
    const result = await pool.query("select * from customers where lower(email) = $1", [value]);
    const customers = await hydrateCustomers(result.rows.map(customerFromRow));
    return customers[0] || null;
  }
  ...
  async function listCustomerTickets(id) {
    ...
    const result = await pool.query(
      `select * from tickets
       where lower(coalesce(nullif(customer_email, ''), data #>> '{customer,email}', data ->> 'customerEmail', data ->> 'email', '')) = $1
...
async function upsertTicket(clientOrPool, ticket) {
  await clientOrPool.query(
    ...
    [
      ...
      normalizeEmail(ticket.customer?.email || ticket.customerEmail || ticket.email || ""),
      ticket.model || "",
      ticket.family || "",
      ticket.source || "",
