...
import { isValidSlaFilter, matchesTicketSlaFilter, normalizeSlaFilter } from "./ticket-sla.mjs";
...
export function parseQueueTicketQuery(searchParams) {
  ...
  const slaValidation = validateSlaFilter(filters);
  if (!slaValidation.ok) return slaValidation;
...
export function filterTicketsForQueueView(tickets, view, context = {}, query = {}) {
  ...
  const filtered = safeTickets
    .filter((ticket) => parseBoolean(mergedFilters.includeMerged, false) || !ticket?.mergedInto)
    ...
    .filter((ticket) => matchesTicketSlaFilter(ticket, mergedFilters.sla, settings))
...
function normalizeQueueFilters(filters = {}) {
  ...
  if (normalizeSlaFilter(filters.sla)) normalized.sla = normalizeSlaFilter(filters.sla);
...
function validateSlaFilter(filters) {
  if (filters.sla && !isValidSlaFilter(filters.sla)) {
    return invalidQueueFilter("sla", "sla must be overdue, due-soon, on-track, closed, or no-due-date.");
  }
  return { ok: true };
}
