import { slaStatusCounts } from "./ticket-sla.mjs";
...
export function buildAnalyticsSummary({
  ...
  const safeTickets = Array.isArray(tickets) ? tickets.filter((ticket) => !ticket?.mergedInto) : [];
  ...
  const slaCounts = slaStatusCounts(safeTickets, settings, { now: generatedAt });
  ...
  return {
    ...
    metrics: {
      ...
      overdueTicketCount: slaCounts.overdue,
      dueSoonTicketCount: slaCounts.dueSoon,
...
function currentAssignmentName(user, settings) {
  return stringValue(user?.assignmentName || user?.repName || user?.displayName || settings.currentUserName || settings.defaultAssignee || "");
}
