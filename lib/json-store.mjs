...
export function createJsonStore({ dataFile, defaultState }) {
  ...
  async function mergeTickets(id, input, context = {}) {
    return queuedTicketMutation((tickets) => {
      const result = mergeTicketsInCollection(tickets, id, input, context);
      const { tickets: _tickets, updatedTickets: _updatedTickets, ...response } = result;
      return { tickets: result.tickets, response };
    });
  }
  ...
  async function listCustomerTickets(id) {
    ...
    const tickets = await ticketCollection();
    return tickets.filter((ticket) => ticketCustomerEmail(ticket) === customer.email);
  }
...
export function filterTickets(tickets, filters = {}) {
  ...
  const includeMerged = parseBoolean(filters.includeMerged, false);
  const limit = clampInt(filters.limit, 1, 500, 100);
  const offset = clampInt(filters.offset, 0, 100000, 0);

  return tickets
    .filter((ticket) => includeMerged || !ticket?.mergedInto)
...
function ticketSearchText(ticket) {
  return [
    ...
    ...(Array.isArray(ticket.mergedFrom) ? ticket.mergedFrom : []),
    ...(Array.isArray(ticket.conversation) ? ticket.conversation.flatMap((message) => [message?.body, message?.mergedFrom, message?.sourceTicketId]) : []),
    ...(Array.isArray(ticket.attachments) ? ticket.attachments.flatMap((attachment) => [attachment?.fileName, attachment?.name, attachment?.downloadUrl, attachment?.mergedFrom, attachment?.sourceTicketId]) : [])
  ].filter(Boolean).join(" ").toLowerCase();
}

function ticketCustomerEmail(ticket) {
...
export function normalizeCustomer(input = {}) {
  ...
  return {
    ...
    purchaseSource: String(input.purchaseSource || "Unknown").trim() || "Unknown",
...
function customerMatchesId(customer, id) {
  ...
  return String(customer.id || "").trim().toLowerCase() === value || normalizeEmail(customer.email) === email;
}
